...
% attack_length_set_DI_sum=zeros(1,split);
% attack_length_set_DI1_sum=zeros(1,split);
attack_length_set_sum=zeros(1,split);
attack_length_set_sum1=zeros(1,split);
...
for i=1:split
    ...
    %attack_length_set_DI=[attack_length_set_DI,sum(attack_length_set)];
    attack_start=randi([50*i 50*i+25],1,1);         %这里可以改变起始时刻t_0的选择
    attack_start_set=[attack_start_set attack_start];
end
 attack_length_set1=[];%存放攻击各个片段的攻击时长
% attack_length_set_DI1=[];%用于存放累计的攻击片段
 attack_start_set1=[];%存放各个片段的攻击起始时刻
for i=1:split
    attack_length1=randsrc(1,1,[extra_length;f]);%当前时刻的攻击时长
    attack_length_set1=[attack_length_set1 attack_length1+basic_length];
end  
% attack_length_set_DI1_sum=attack_length_set_DI1_sum+attack_length_set_DI1;
% attack_length_set_DI_sum=attack_length_set_DI_sum+attack_length_set_DI;
attack_length_set_sum=attack_length_set_sum+attack_length_set;
attack_length_set_sum1=attack_length_set_sum1+attack_length_set1;
...
attack_length_set_DI=zeros(1,split);%用于存放累计的攻击片段
middle=0;
attack_length_set_DI1=zeros(1,split);%用于存放累计的攻击片段
middle1=0;
for i=1:split
    sum=0;
    sum_center=0;
    ...
    attack_length_set_DI(i)=middle+attack_length_set_sum(i);
    middle=attack_length_set_DI(i);
    attack_length_set_DI1(i)=middle1+attack_length_set_sum1(i);
    middle1=attack_length_set_DI1(i);
end

figure
plot(1:split,optimal_DI_cunchu_aver,'-^',1:split,center_DI_cunchu_aver,'-.>',1:split,random_DI_cunchu_aver,'--o',1:split,attack_length_set_DI/repeat_times,'--<',1:split,attack_length_set_DI/repeat_times,':>','linewidth',2);
legend({'Optimal encryption scheduling','Centralized encryption scheduling','Random encryption scheduling','Encoding strategy in [24]','Watermarking strategy in [20]'},  'Interpreter','latex', 'FontSize', 18, 'location', 'northeast');
xlabel('Times of subattack' ,'Interpreter','latex','FontSize',18);
ylabel({'$DI\left(T_r,\theta\right)$'}, 'Interpreter','latex','FontSize',18);
set(gca,'FontSize',18);%坐标轴字体大小
